...
<div>
    ...
    <x-theme::card class="mb-4">
        <h2 class="mb-1 text-lg font-semibold text-gray-900 dark:text-white">Purchases</h2>
        <p class="mb-4 text-sm text-gray-500 dark:text-gray-400">Customers who purchased this resource or were granted access.</p>

        @if($licenses->isEmpty())
            <x-theme::empty-state title="{{ __('marketplace::messages.no_licenses') }}" description="Purchases appear here when someone buys the resource, downloads a free resource, or you grant access manually." />
        @else
