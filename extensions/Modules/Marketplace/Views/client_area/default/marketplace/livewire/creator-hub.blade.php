...
@php
    ...
    $revenue = (float) MarketplaceSale::query()->where('seller_id', $userId)->completed()->sum('amount');
...
<div>
    <div class="mb-6 grid gap-4 sm:grid-cols-3">
        <div class="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
            ...
            <div class="mt-1 text-2xl font-semibold text-gray-900 dark:text-white">{{ $resources->total() }}</div>
        </div>
        <div class="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
    ...
    @if($resources->isEmpty())
        <x-theme::empty-state title="No resources yet" description="Publish your first integration, theme, or gateway." />
    @else
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    @foreach($resources as $resource)
                        <tr wire:key="studio-{{ $resource->id }}">
                            ...
                            <td class="px-4 py-3 text-xs text-gray-500 dark:text-gray-400">{{ $resource->views_count }} views · {{ $resource->downloads_count }} dl</td>
